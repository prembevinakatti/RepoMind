...
const RepoDetailsPage = () => {
  ...
  const fetchRepoDetails = async () => {
    try {
      const res = await fetch(`http://localhost:3000/api/repomind/repo/${id}`);
      const data = await res.json();
      setRepoData(data.data);
    } catch (err) {
      alert("Failed to fetch repo details!");
    }
    setLoading(false);
  };

  useEffect(() => { fetchRepoDetails(); }, []);
        ...
        <div className="mt-3 bg-white/70 p-4 rounded-xl shadow space-y-3">
          {repoData.documentation.map((doc, i) => (
            <div key={i}>
              ...
              <p className="text-sm text-gray-700">{doc.summary || "No summary available."}</p>
            </div>
          ))}
        </div>
